    ...
    for row in res:
        ...
        for row2 in siminvestor_id: 
            for scoretmp in score:
                if row2 == scoretmp['investor_id']:
                    ...
                    siminvestor.append({u'investor_name': siminvestor_name.decode('Windows-1252'), u'investor_score': scoretmp['similarity']})
        ...
        if len(tmp) > 0 and len(tmp[0][1]) > 2:
            ...
            if float(fund) < 0.01:
                ...
                fund = '$'+str(fund)+'M'
        if len(tmp) > 0:
            #res2.append({u'startup_score': row[0], u'startup_id': row[1], u'startup_name': tmp[0][0].decode('Windows-1252')})

            res2.append({u'startup_score': row[0], u'startup_id': row[1],u'startup_name': tmp[0][0].decode('Windows-1252'), u'startup_market': tmp[0][1].decode('Windows-1252'),u'logolink': tmp[0][2].decode('Windows-1252'),u'startupSignal': tmp[0][3], u'angellink': tmp[0][4].decode('Windows-1252'), u'funding': fund, u'investors': sorted(siminvestor, key=itemgetter('investor_score'),reverse=True)})  
            #res2.append({u'startup_score': row[0], u'startup_id': row[1],u'startup_name': tmp[0][0].decode('Windows-1252'), u'startup_market': tmp[0][1].decode('Windows-1252'),u'logolink': tmp[0][2].decode('Windows-1252'),u'startupSignal': tmp[0][3], u'angellink': tmp[0][4].decode('Windows-1252'), u'funding': fund})  
